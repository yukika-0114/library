...
## シークレットルーム(メールアドレス不要で使う)

友だちがメールアドレスを入力せずに、共通パスワードだけでライブラリに参加できる機能です。

### 有効にする手順(1回だけ)

1. Supabase →「**Authentication**」→「**Sign In / Providers**」を開く
2. 「**Anonymous Sign-Ins**」(匿名ログイン)を **ON** にする
3. SQL Editorを開き、`supabase/migration_secret_room.sql` の中身を貼り付けて実行する
   (すでに `schema.sql` を実行済みの場合、全体を再実行するとエラーになるため、この差分ファイルだけを実行してください)

### 使い方

- **管理者(あなた)**:ログイン画面の下部「新規作成」タブは使わず、通常通りメールでログイン →「新しいライブラリを作る」で「シークレットルームにする」にチェックを入れ、共通パスワードを設定して作成
- **参加する人**:ログイン画面の「メールを使わずシークレットルームで入る」を選び、共通パスワードだけを入力

パスワードはハッシュ化してデータベースに保存され、通常の方法では読み出せないようになっています。参加した人はメールアドレス不要の匿名アカウントとして扱われますが、写真やデータはこれまでと同様にサーバー上にきちんと保存されるため、再読み込みしても消えません(ただし、匿名アカウントの状態はそのブラウザ内に保存されるため、別の端末では改めて同じパスワードで入室し直す必要があります)。
